...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
   if (node == nullptr) {
    ...
    else { //We have found a node -> datum > val, a potential solution
      Node * left_check = min_greater_than_impl(node -> left, val, less);
      if(left_check == nullptr){
      //if(left_check is a nullptr) {
      //We did not find anything better in the left subtree
        return node;
      //return node currently at; }

      }
      else {
        //We did find something better, left_check is equal to some node
        //return left_check
        return left_check;
      }
