...
private:
  ...
  class PairComp {
    ...
    public:
    PairComp(Key_compare pairs = Key_compare())
      : less(pairs) { }
...
public:
  ...
  Iterator find(const Key_type& k) const{
    auto found_it = entries.find(std::make_pair(k, Value_type()));
  ...
  Value_type& operator[](const Key_type& k){
    ...
    auto new_key = insert(std::make_pair(k, Value_type()));
    return new_key.first->second;
